...
summary: TiDB v6.0.0 在 TPC-C 性能上比 v5.4.0 提升了 24.20%，表现更好。测试环境为 AWS EC2，硬件配置包括 PD、TiKV、TiDB 和 TPC-C 实例。软件版本为 v5.4.0、v6.0.0，配置参数相同。测试方案包括部署、数据库创建、数据导入和压力测试。结果显示，v6.0.0 在不同并发数下的 tpmC 均有提升，最高达到 26.97%。
---
